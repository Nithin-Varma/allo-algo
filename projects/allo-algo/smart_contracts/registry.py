...
#         self.anchorToProfile = algopy.LocalState(algopy.Bytes)
     
#     @algopy.subroutine
#     def _getProfileByAnchor(self,anchor:algopy.Account) -> Profile:
#         result, exists = self.anchorToProfile.maybe(anchor)
#         assert exists, "no data for account"
#         return Profile.from_bytes(result)
#     @arc4.abimethod()
#     def getProfileByAnchor(self, anchor: algopy.Account) -> Profile:
#         return self._getProfileByAnchor(anchor)
...
#          arc4.abi_call(Anchor.createAndOptIn,algopy.Txn.sender,algopy.Txn.application_id,app_id=anchor_app.id,fee=1000)
#          profile = Profile(
...
#              members= arc4.DynamicArray(arc4.Address())
#             )
#          self.anchorToProfile[anchor_app.address] = profile.bytes
#          return id
    
#     @arc4.abimethod()
#     def addMember(self,anchor:algopy.Account,member:arc4.Address) -> None:
#         self._onlyProfileOwner(anchor)
#         profile = self._getProfileByAnchor(anchor)
#         profile.members.append(member)
#         self.anchorToProfile[anchor] = profile.copy().bytes
    
#     @arc4.abimethod()
#     def updateProfileName(self,anchor:algopy.Account,name:arc4.String) -> None:
#         self._onlyProfileOwner(anchor)
#         profile = self._getProfileByAnchor(anchor)
#         profile.name = name
#         self.anchorToProfile[anchor] = profile.copy().bytes
#     @arc4.abimethod()
#     def updateProfileMetadata(self,anchor:algopy.Account,metadata:arc4.String) -> None:
#         self._onlyProfileOwner(anchor)
#         profile = self._getProfileByAnchor(anchor)
#         profile.metadata = metadata
#         self.anchorToProfile[anchor] = profile.copy().bytes
#     @arc4.abimethod()
#     def isOwnerOrMemberOfProfile(self,anchor:algopy.Account,address:arc4.Address) -> bool:
#         profile : Profile = self._getProfileByAnchor(anchor)
...
#     @algopy.subroutine
#     def _onlyProfileOwner(self,anchor:algopy.Account) -> None:
#         profile = Profile.from_bytes(self.getProfileByAnchor(anchor).bytes)
